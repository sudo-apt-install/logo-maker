...
function renderShape(shape, data) {
  ...
  switch (shape) {
    case "Circle":
      shapeSelection = `<svg viewbox="-100 -80 500 500">
      <circle cx="50" cy="50" r="50" fill="${data.shapeColor}" />
      <text x="50" y="65" font-size="300%" text-anchor="middle" fill="${data.color}">123</text>
    </svg> `;
      break;
    ...
    case "Square":
      shapeSelection = `<svg>
      <rect x="0" y="0" width="50dvw" height="50dvh" fill="${data.shapeColor}" /><text x="50%" y="50%" font-size="60" text-anchor="middle" fill="${data.color}">svg</text>
    </svg>`;
      break;
    default:
      break;
